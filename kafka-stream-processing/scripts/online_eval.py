...
def read_configs(config_path):
    ...
    return dbConf, kafkaConf,s3Conf
...
def evaluate_ratings(recommendations_table, rating_event_table, consumer, db_con,model_id):
    ...
    for message in consumer:
        ...
        if(len(values) == 3 and '/rate/' in values[2]):
            ...
            if validate(ts, user_id, movie_id, rating, year):
                ...
                user_liking = get_user_liking(int(rating))
                ...
                update_clause = recommendations_table.update().where(
                    ...
                    (recommendations_table.c.rating == 0) ).values({'rating':user_liking})
                
                try:
                    db_con.execute_query(update_clause)
                except:
                    pass
...
def validate(timestamp, user_id, movie_id, rating,year):
    ...
    '''
    try:
        if(valiate_year(year) and validate_timestamp(timestamp) and validate_rating(rating) and validate_userid(user_id) and  match_pattern(movie_id,r"(.*\+\d{4}$)")):
            return True
...
if __name__ == "__main__":
    ...
    # Read configs
    dbConf, kafkaConf,s3Conf = read_configs('config/config.json')
    ...
    # Begin dump process
    evaluate_ratings(recommendations_table,rating_event_table, consumer, db,model_id)
